...
#         import pdb; pdb.set_trace()
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            dataset_full = datasets.ImageFolder(root='./data/Batch 2/Train/', transform=self.tf_compose)
        ...
        if stage == 'test' or stage is None:
            self.dataset_test = datasets.ImageFolder(root='./data//Batch 2/Test/', transform=self.tf_compose)
...
try:
    trainer = pl.Trainer(gpus=2,max_epochs=50,default_root_dir='./custom_logs')
...
get_ipython().run_line_magic('tensorboard', '--logdir custom_logs/ --port=6011')
