...
try:
    trainer = pl.Trainer(gpus=1,max_epochs=100,default_root_dir='./collection5_logs_resnet', callbacks=[EarlyStopping(monitor="val_loss", mode="min", min_delta=0.00)])
except Exception as e:
    # most likely due to GPU, so fallback to non GPU
    print(e)
    trainer = pl.Trainer(max_epochs=100,default_root_dir='./collection5_logs_resnet', callbacks=[EarlyStopping(monitor="val_loss", mode="min")])
...
plt.tight_layout()
plt.savefig('collection5-resnet.png')
