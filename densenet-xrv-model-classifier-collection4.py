...
#         import pdb; pdb.set_trace()
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            dataset_full = datasets.ImageFolder(root='./data/Collection 4/Train/', transform=self.tf_compose)
        ...
        if stage == 'test' or stage is None:
            self.dataset_test = datasets.ImageFolder(root='./data/Collection 4/Test/', transform=self.tf_compose)
...
try:
    trainer = pl.Trainer(gpus=1,max_epochs=100,default_root_dir='./collection4_logs_densenet', callbacks=[EarlyStopping(monitor="val_loss", mode="min", min_delta=0.00)])
except Exception as e:
    # most likely due to GPU, so fallback to non GPU
    print(e)
    trainer = pl.Trainer(max_epochs=100,default_root_dir='./collection4_logs_densenet', callbacks=[EarlyStopping(monitor="val_loss", mode="min")])
...
plt.tight_layout()
plt.savefig('collection4-densenet.png')
